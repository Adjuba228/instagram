...
<html lang="en">
  ...
  <body>
	...
	<div class="col-6 mx-auto" > <!--ГЛАВНЫЙ БЛОК-->
		...
		<div class="row"> 
				...
				<div class="mt-5 border rounded">   
					...
					<div class="col-12 py-2">  <!--текстовые элементы поста-->
						...
						<div class="col-2">
	 					...
	 					<form action="update.php" method="GET" class="upd" style="display: none">
							<input type="" name="text" value="<?php echo $stroka["text"]?>"
							>
							<input type="" name="img" value="<?php echo $stroka["img"]?>" >
							<input type="" name="id" value="<?php echo $stroka["id"]?>">
							<button class="btn bg-primary save" >Исправить</button>
			...
			<div class="col-4 mt-5">  <!--ПРАВЫЙ БЛОК. ПОДПИСКИ И ТД-->
				...
				<button class="btn "><a href="admin.php" class="text-dark">Добавить пост</a></button>
	...
	<script type="text/javascript">
		...
		let btn_fix = document.querySelectorAll(".save");
		let btn_update = document.querySelectorAll(".btn-success");
		for (let i = 0; i < 5 ; i++) { 
			btn_update[i].onclick = function() {
				upd[i].style.display = 'block';
			}
			btn_fix[i].onclick = function() {
